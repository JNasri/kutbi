...
const atlasPosition = (index: number) =>
  `${(index % 4) * (100 / 3)}% ${Math.floor(index / 4) * 50}%`;
...
export default function DiscoverSaudi({ copy }: { copy: DiscoverCopy }) {
  ...
  const active =
    copy.categories.find((category) => category.id === activeId) ??
    copy.categories[0];
  ...
  return (
    <section id="discover" className="content-section discover-section">
      ...
      <div className="discover-shell content-wrap">
        <div className="category-tabs" role="tablist" aria-label={copy.title}>
          ...
          {active.items.map((item) => {
            ...
            return (
              <article className="destination-card" key={item.name}>
                <div
                  className="atlas-image destination-image"
                  role="img"
                  aria-label={item.name}
                  style={{ backgroundPosition: atlasPosition(item.index) }}
                />
