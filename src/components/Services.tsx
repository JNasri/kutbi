...
const darkServiceImages: Record<string, string> = {
  ...
  "/images/service-transportation.jpeg":
    "/images/service-transportation-dark.jpeg",
...
export default function Services({ copy, theme }: ServicesProps) {
  return (
    <section id="services" className="content-section services-section">
      ...
      <div className="service-grid content-wrap">
        {copy.cards.map((card) => (
          <article className="service-card" key={card.image}>
            <Link
              className="service-card-link"
              to={serviceDestinations[card.image] ?? "/services"}
              aria-label={card.title}
            >
              <img
                src={
                  theme === "dark"
                    ? (darkServiceImages[card.image] ?? card.image)
                    : card.image
                }
                alt={card.title}
                loading="lazy"
