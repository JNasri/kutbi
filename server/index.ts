...
const uploadRoot = path.resolve(
  process.env.UPLOAD_DIR ?? path.join(projectRoot, "uploads", "journals"),
);
...
const localUploadPath = (url: string) =>
  url.startsWith("/uploads/journals/")
...
async function removeStoredUploads(urls: string[]) {
  const removals = await Promise.allSettled(
    urls.map(async (url) => {
      if (await deleteFileFromS3(url)) return;
  ...
  removals.forEach((result) => {
    if (result.status === "rejected") {
      console.error("Unable to remove a stored journal image:", result.reason);
    }
  });
}
...
app.use("/uploads/journals", (_request, response) =>
  ...
  async (request, response, next) => {
    try {
      ...
      response.status(201).json({
        mainImage: mainImage ? uploadedUrls[0] : null,
        galleryImages: uploadedUrls.slice(galleryOffset),
      });
    } catch (error) {
      next(error);
    }
    ...
    const result = await pool.query(
      `INSERT INTO blog_posts
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10::varchar,$11,
         CASE WHEN $10::varchar = 'published' THEN COALESCE($12::timestamptz, NOW()) ELSE NULL END)
       RETURNING *`,
      [
        post.slug,
        post.title_ar,
        ...
        post.content_en,
        post.image_url,
        JSON.stringify(post.gallery_images),
        post.status,
        request.user!.id,
  ...
  async (request, response, next) => {
    try {
      ...
      const result = await pool.query(
        ...
        published_at=CASE
          WHEN $10::varchar = 'draft' THEN NULL
      ...
      const previousImages = [
        ...
        ...(existing.rows[0].gallery_images ?? []),
      ];
...
app.use(
  (
    ...
    if (
      ...
      error.message === "Only JPG, PNG, WebP, and AVIF images are supported."
    ) {
    ...
    if (
      ...
      response
        .status(409)
        .json({ error: "That slug or username is already in use." });
      return;
    }
    console.error(error);
    response
